...
import (
	...
	initoptions "github.com/karmada-io/karmada/pkg/karmadactl/cmdinit/options"
...
func TestCommandInitOption_newGenCerts(t *testing.T) {
	tmpPath := "/tmp/pki"
	defer os.RemoveAll(tmpPath)
	opt := &CommandInitOption{
		CertValidity:       365 * 24 * time.Hour,
...
// TestInit_GeneratesFullSetAndIssuers verifies that Init() builds a full
// certificate set and that certs are signed by the expected CA (main, etcd-ca, front-proxy-ca).
func TestInit_GeneratesFullSetAndIssuers(t *testing.T) { //nolint:funlen
	tmpPath := "./tmp-full-pki"
	defer os.RemoveAll(tmpPath)
	...
	// CA files
	for _, f := range []string{"ca.crt", "ca.key", "front-proxy-ca.crt", "front-proxy-ca.key", "etcd-ca.crt", "etcd-ca.key"} {
		if _, err := os.Stat(filepath.Join(tmpPath, f)); err != nil {
			t.Fatalf("expected CA file missing: %s: %v", f, err)
		}
	}

	// Expected cert names generated by Init() builders
	expectedNames := []string{
		// etcd
		initoptions.EtcdServerCertAndKeyName,
		initoptions.EtcdClientCertAndKeyName,
		// apiserver + admin + etcd-client + front-proxy-client
		initoptions.KarmadaApiServerCertAndKeyName,
		initoptions.KarmadaApiServerEtcdClientCertAndKeyName,
		initoptions.FrontProxyClientCertAndKeyName,
		// aggregated apiserver
		initoptions.KarmadaAggregatedApiServerCertAndKeyName,
		initoptions.KarmadaAggregatedApiServerClientCertAndKeyName,
		initoptions.KarmadaAggregatedApiServerEtcdClientCertAndKeyName,
		// webhook
		initoptions.KarmadaWebhookCertAndKeyName,
		initoptions.KarmadaWebhookClientCertAndKeyName,
		// search
		initoptions.KarmadaSearchCertAndKeyName,
		initoptions.KarmadaSearchClientCertAndKeyName,
		initoptions.KarmadaSearchEtcdClientCertAndKeyName,
		// controller-manager client
		initoptions.KarmadaControllerManagerClientCertAndKeyName,
		// scheduler
		initoptions.KarmadaSchedulerGrpcCertAndKeyName,
		initoptions.KarmadaSchedulerClientCertAndKeyName,
		// descheduler
		initoptions.KarmadaDeschedulerClientCertAndKeyName,
		initoptions.KarmadaDeschedulerGrpcCertAndKeyName,
	}

	issuerFor := func(name string) string {
		switch name {
		case initoptions.EtcdServerCertAndKeyName,
			initoptions.EtcdClientCertAndKeyName,
			initoptions.KarmadaApiServerEtcdClientCertAndKeyName,
			initoptions.KarmadaAggregatedApiServerEtcdClientCertAndKeyName,
			initoptions.KarmadaSearchEtcdClientCertAndKeyName:
			return initoptions.EtcdCaCertAndKeyName
		case initoptions.FrontProxyClientCertAndKeyName:
			return initoptions.FrontProxyCaCertAndKeyName
		default:
			return "karmada"
		}
	}
	...
	for _, name := range expectedNames {
		crt := filepath.Join(tmpPath, fmt.Sprintf("%s.crt", name))
		key := filepath.Join(tmpPath, fmt.Sprintf("%s.key", name))
		if _, err := os.Stat(crt); err != nil {
			t.Fatalf("missing cert %s: %v", crt, err)
		}
		if _, err := os.Stat(key); err != nil {
			t.Fatalf("missing key %s: %v", key, err)
		}
		issuerCN, err := readCertIssuerCN(crt)
		if err != nil {
			t.Fatalf("parse cert %s failed: %v", crt, err)
		}
		want := issuerFor(name)
		if issuerCN != want {
...
func readCertIssuerCN(path string) (string, error) {
	...
	return c.Issuer.CommonName, nil
}
